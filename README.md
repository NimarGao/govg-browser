...
- 🛡️ **账号安全护栏**：
  - 不伪装 Chrome，不做浏览器指纹绕过或自动化隐藏。
  - Google 账号登录、账号选择和 OAuth 页面不会在应用内嵌视图中打开，会自动转到系统默认浏览器处理。
  - 密码保存脚本不会在 Google 账号认证页面捕获表单，避免触碰高风险账号登录链路。
...
│   └── webview-preload.cjs  # 网页 preload (快捷键捕获与普通表单保存提示)
├── src/
...
Google 明确可能阻止嵌入在其他应用中的浏览器登录账号。Govg Browser 因此不承接 Google 账号登录和 OAuth 授权流程：检测到 `accounts.google.com`、Google 登录页或 OAuth 页面时，会调用系统默认浏览器打开。

这不是体验妥协，而是账号安全边界。不要通过伪造 User-Agent、隐藏自动化特征或注入脚本去绕过 Google 风控；这类做法可能让账号更容易被标记为异常。
